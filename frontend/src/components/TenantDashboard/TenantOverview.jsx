...
function TenantOverview() {
  ...
  const loadRequests = async () => {
    const result = await Axios.get("http://localhost:5000/request")
    setRequestsData(result.data);
  }

  const loadAnnouncements = async () => {
    const result = await Axios.get("http://localhost:5000/announcements")
  ...
  return (
    <div>
      ...
      <div className='my-6 border-l-4 border-green-700 w-11/12 py-11 flex justify-between items-center bg-white drop-shadow rounded-[8px] p-6'>
        ...
        <div>
          <button className='bg-green-700 text-white tracking-wider rounded-full px-10 py-4'>Make payment</button>
        </div>
      </div>
      ...
      <div className='flex justify-between gap-6'>
        <div className='w-[50%] h-[300px] overflow-auto bg-white drop-shadow rounded-[8px] p-6 flex flex-col justify-between'>
          ...
          <div className=''>
            ...
            {requestsData && requestsData.map((data) => (
              <div className='mt-6 flex items-center gap-4'>
                <div className='border-2 border-green-700 bg-green-100 text-green-700 text-[22px] p-3 rounded-[8px]'>
                  <FiTool />
                </div>
                <div>
                  <h6 className='text-[16px] font-semibold'>{data.requestSubject}</h6>
                  <p className='text-[12px] font-medium text-gray-500'>{data.requestType}</p>
                </div>
              </div>
            ))}
          </div>

          <button onClick={() => navigate('/tenantdashboard/requests/new-request')} className='bg-green-700 text-white mt-6 tracking-wider rounded-full px-10 py-2 self-start'>Create request</button>
        </div>
        ...
        <div className='w-[50%] h-[300px] overflow-auto bg-white drop-shadow rounded-[8px] p-6'>
          <div className=''>
            ...
            {announcementsData && announcementsData.map((data) => (
              <div className='mt-6 flex items-center gap-4'>
                <div className='border-2 border-green-700 bg-green-100 text-green-700 text-[22px] p-3 rounded-[8px]'>
                  <FiTool />
                </div>
                <div>
                  <h6 className='text-[16px] font-semibold'>{data.announcementMessage}</h6>
                  <p className='text-[12px] font-medium text-gray-500'>{data.announcementdate}</p>
                </div>
              </div>
            ))}
          </div>
